...
public class b2Collision{
	...
	static public function EdgeSeparation(poly1:b2PolyShape, edge1:int, poly2:b2PolyShape):Number
	{
		...
		var vertexIndex11:int = edge1;
		var vertexIndex12:int = edge1 + 1 == count1 ? 0 : edge1 + 1;
		
		// Get the normal of edge1.
		//var normalLocal1:b2Vec2 = b2Math.b2CrossVF( b2Math.SubtractVV(vert1s[vertexIndex12], vert1s[vertexIndex11] ), 1.0);
		var normalLocal1X:Number = vert1s[vertexIndex12].x - vert1s[vertexIndex11].x;
		var normalLocal1Y:Number = vert1s[vertexIndex12].y - vert1s[vertexIndex11].y;
		var tX:Number = normalLocal1X;
		normalLocal1X = normalLocal1Y;
		normalLocal1Y = -tX;
		// ^^^^^ normalLocal1.CrossVF(1.0);
		
		var invLength:Number = 1.0 / Math.sqrt(normalLocal1X*normalLocal1X + normalLocal1Y*normalLocal1Y);
		normalLocal1X *= invLength;
		normalLocal1Y *= invLength;
		// ^^^^^ normalLocal1.Normalize();
		
		//var normal:b2Vec2 = b2Math.b2MulMV(poly1.m_R, normalLocal1);
		var normalX:Number = normalLocal1X;
		var normalY:Number = normalLocal1Y;
		tX = normalX;
		...
		//var v1:b2Vec2 = b2Math.AddVV(poly1.m_position, b2Math.b2MulMV(poly1.m_R, vert1s[vertexIndex11]));
		var v1:b2Vec2 = new b2Vec2();
		v1.SetV(vert1s[vertexIndex11]);
		v1.MulM(poly1.m_R);
		v1.Add(poly1.m_position);
		
		//var v2:b2Vec2 = b2Math.AddVV(poly2.m_position, b2Math.b2MulMV(poly2.m_R, vert2s[vertexIndex2]));
		...
		var separation:Number = v2.x * normalX + v2.y * normalY;
		return separation;
	}
	...
	// from poly1.
	static public function FindMaxSeparation(edge:Array /*int ptr*/, poly1:b2PolyShape, poly2:b2PolyShape):Number
	{
		var count1:int = poly1.m_vertexCount;
		var vert1s:Array = poly1.m_vertices;
		...
		var prevFaceIndex:int = vertexIndex1 - 1 >= 0 ? vertexIndex1 - 1 : count1 - 1;
		var sPrev:Number = EdgeSeparation(poly1, prevFaceIndex, poly2);
		if (sPrev > 0.0)
		{
			return sPrev;
		}
		
		var nextFaceIndex:int = vertexIndex1;
		var sNext:Number = EdgeSeparation(poly1, nextFaceIndex, poly2);
		if (sNext > 0.0)
		{
			return sNext;
		}
		...
		while (true)
		{
			...
			var separation:Number = EdgeSeparation(poly1, edgeIndex, poly2);
			if (separation > 0.0)
			{
				return separation;
			}
	...
	static public function FindIncidentEdge(c:Array, poly1:b2PolyShape, edge1:int, poly2:b2PolyShape)
	{
		var count1:int = poly1.m_vertexCount;
		var vert1s:Array = poly1.m_vertices;
		var count2:int = poly2.m_vertexCount;
	...
	static public function b2CollidePoly(manifold:b2Manifold, polyA:b2PolyShape, polyB:b2PolyShape){
		manifold.pointCount = 0;
		
		var edgeA:int = 0;
		var edgeAOut:Array = [edgeA];
		var separationA:Number = FindMaxSeparation(edgeAOut, polyA, polyB);
		edgeA = edgeAOut[0];
		if (separationA > 0.0)
			return;
		...
		var separationB:Number = FindMaxSeparation(edgeBOut, polyB, polyA);
		edgeB = edgeBOut[0];
		if (separationB > 0.0)
			return;
	...
	static public function b2CollideCircle(manifold:b2Manifold, circle1:b2CircleShape, circle2:b2CircleShape)
	{
		manifold.pointCount = 0;
	...
	static public function b2CollidePolyAndCircle(manifold:b2Manifold, poly:b2PolyShape, circle:b2CircleShape)
	{
		manifold.pointCount = 0;
		...
		var vertIndex2:int = poly.m_nextVert[vertIndex1];
