...
public class b2PrismaticJoint extends b2Joint
{
	...
	public function SetMotorForce(force:Number)
	{
	...
	public override function PrepareVelocitySolver(){
		...
		var r2:b2Vec2 = b2Math.b2MulMV(b2.m_R, m_localAnchor2);
		//tMat = b2.m_R;
		//var r1X:Number = tMat.col1.x * m_localAnchor2.x + tMat.col2.x * m_localAnchor2.y;
		//var r1Y:Number = tMat.col1.y * m_localAnchor2.x + tMat.col2.y * m_localAnchor2.y
		...
		// J = [-ay1 -cross(d+r1,ay1) ay1 cross(r2,ay1)]
		var ay1:b2Vec2 = b2Math.b2MulMV(b1.m_R, m_localYAxis1);
		
		var e:b2Vec2 = b2Math.SubtractVV(b2Math.AddVV(b2.m_position , r2) , b1.m_position);
		
		m_linearJacobian.Set(ay1.Negative(), -b2Math.b2CrossVV(e, ay1), ay1, b2Math.b2CrossVV(r2, ay1));
		...
		if (m_enableLimit || m_enableMotor)
		{
			...
			var ax1:b2Vec2 = b2Math.b2MulMV(b1.m_R, m_localXAxis1);
			m_motorJacobian.Set(ax1.Negative(), -b2Math.b2CrossVV(e, ax1), ax1, b2Math.b2CrossVV(r2, ax1));
			...
			if (m_enableLimit)
			{
				var d:b2Vec2 = b2Math.SubtractVV(e , r1);	// p2 - p1
				var jointTranslation:Number = b2Math.b2Dot(ax1, d);
		...
		{
			//var P1:b2Vec2 = m_linearImpulse * m_linearJacobian.linear1 + (m_motorImpulse + m_limitImpulse) * m_motorJacobian.linear1;
			var P1:b2Vec2 = b2Math.AddVV(b2Math.MulFV(m_linearImpulse, m_linearJacobian.linear1), b2Math.MulFV((m_motorImpulse + m_limitImpulse), m_motorJacobian.linear1));
			var P2:b2Vec2 = b2Math.AddVV(b2Math.MulFV(m_linearImpulse, m_linearJacobian.linear2), b2Math.MulFV((m_motorImpulse + m_limitImpulse), m_motorJacobian.linear2));
			...
			b1.m_linearVelocity.Add( b2Math.MulFV( invMass1 , P1 ) );
			b1.m_angularVelocity += invI1 * L1;
			
			b2.m_linearVelocity.Add( b2Math.MulFV( invMass2 , P2 ) );
	...
	public override function SolveVelocityConstraints(step:b2StepInfo){
		...
		b1.m_linearVelocity.Add( b2Math.MulFV((invMass1 * linearImpulse) , m_linearJacobian.linear1) );
		b1.m_angularVelocity += invI1 * linearImpulse * m_linearJacobian.angular1;
		
		b2.m_linearVelocity.Add( b2Math.MulFV((invMass2 * linearImpulse) , m_linearJacobian.linear2) );
		...
		if (m_enableMotor && m_limitState != e_equalLimits)
		{
			...
			b1.m_linearVelocity.Add( b2Math.MulFV((invMass1 * motorImpulse), m_motorJacobian.linear1 ));
			b1.m_angularVelocity += invI1 * motorImpulse * m_motorJacobian.angular1;
			
			b2.m_linearVelocity.Add( b2Math.MulFV((invMass2 * motorImpulse), m_motorJacobian.linear2 ));
		...
		if (m_enableLimit && m_limitState != e_inactiveLimit)
		{
			...
			b1.m_linearVelocity.Add( b2Math.MulFV((invMass1 * limitImpulse) , m_motorJacobian.linear1 ) );
			b1.m_angularVelocity += invI1 * limitImpulse * m_motorJacobian.angular1;
			
			b2.m_linearVelocity.Add( b2Math.MulFV((invMass2 * limitImpulse) , m_motorJacobian.linear2 ) );
	...
	public override function SolvePositionConstraints():Boolean{
		...
		var invMass1:Number = b1.m_invMass, invMass2 = b2.m_invMass;
		var invI1:Number = b1.m_invI, invI2 = b2.m_invI;
		
		r1 = b2Math.b2MulMV(b1.m_R, m_localAnchor1);
		r2 = b2Math.b2MulMV(b2.m_R, m_localAnchor2);
		//p1 = b1.m_position + r1;
		p1.SetV(r1);
		p1.Add(b1.m_position);
		//p2 = b2.m_position + r2;
		p2.SetV(r2);
		p2.Add(b2.m_position);
		d = b2Math.SubtractVV(p2, p1);
		var ay1:b2Vec2 = b2Math.b2MulMV(b1.m_R, m_localYAxis1);
		
		// Solve linear (point-to-line) constraint.
		var linearC:Number = b2Math.b2Dot(ay1, d);
		...
		b1.m_position.Add( b2Math.MulFV((invMass1 * linearImpulse), m_linearJacobian.linear1));
		b1.m_rotation += invI1 * linearImpulse * m_linearJacobian.angular1;
		//b1->m_R.Set(b1->m_rotation); // updated by angular constraint
		b2.m_position.Add( b2Math.MulFV((invMass2 * linearImpulse), m_linearJacobian.linear2));
		...
		if (m_enableLimit && m_limitState != e_inactiveLimit)
		{
			...
			b1.m_position.Add( b2Math.MulFV((invMass1 * limitImpulse) , m_motorJacobian.linear1 ) );
			b1.m_rotation += invI1 * limitImpulse * m_motorJacobian.angular1;
			b1.m_R.Set(b1.m_rotation);
			b2.m_position.Add( b2Math.MulFV((invMass2 * limitImpulse) , m_motorJacobian.linear2 ) );
